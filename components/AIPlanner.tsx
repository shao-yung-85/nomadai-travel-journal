    ...
    <div className="flex flex-col h-full bg-paper pt-safe">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 pb-24">
        {messages.map((msg, idx) => (
            <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-slide-up`}>
                <div className={`max-w-[85%] p-4 rounded-2xl text-sm font-medium leading-relaxed shadow-sm whitespace-pre-line ${
                    msg.role === 'user' 
      ...
      <div className="p-4 bg-white/90 backdrop-blur border-t border-sand pb-32">
          ...
          <div className="flex gap-2 mt-3 overflow-x-auto no-scrollbar pb-2">
             {["Tokyo 5 Days", "Bangkok Food", "Paris Honeymoon"].map(suggestion => (
                 <button 
                    key={suggestion}
